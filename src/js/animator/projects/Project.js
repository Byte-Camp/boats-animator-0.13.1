(function () {
  ...
  class Project {
    ...
    async showExportFrameDirDialog() {
      let { folderPath, files } = await ipcRenderer.invoke("settings:show-export-frame-dir-dialog");
      console.log(`Export frame dir changed to: ${folderPath}`);

      if (folderPath) {
        this._updateExportFrameDirText(folderPath);

        if (files.length > 0) {
            if (!this.currentTake) {
              this.currentTake = new Take(1, folderPath);
            } else {
